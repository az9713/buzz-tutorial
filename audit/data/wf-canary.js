...
**Path convention:** in your \`file\` fields, give paths relative to the repo root WITHOUT the \`${ROOT}/\` prefix â€” e.g. \`crates/buzz-relay/src/lib.rs\`, not \`${ROOT}/crates/buzz-relay/src/lib.rs\`. But when you open files with your tools, you must include the \`${ROOT}/\` prefix.
...
- \`kind\`, \`created_at\`, and tag values are attacker-chosen â€” including timestamps far in the future or past, duplicate tags, and deeply nested or enormous tag arrays;
...
The repo's own NIP documentation is in \`${ROOT}/docs/nips/\` and its stated security posture in \`${ROOT}/SECURITY.md\` â€” consult them when a rule's intent is unclear rather than guessing.`
...
You are performing a DEEP audit of one slice. A shallow breadth pass has already run; **your job is to find what it missed.** Read the slice's code thoroughly, including the paths it calls into and the callers that reach it â€” following a call out of the slice is expected and encouraged.
...
That skipped list is a hint, not a boundary â€” the interesting defect is often in what a hurried reviewer passed over.
